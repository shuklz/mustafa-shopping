...
- **State** lives in the `state` object (`catalog`, `theme`, `extras`) plus
  module-level `currentSearch`, `currentFilter`, `selectedOnly`.
...
- **Tabs (pills)**: built from catalog categories, with `All Items` first and
  ...
  per-category **totals** appear in each list group header instead.
